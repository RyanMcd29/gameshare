...
db.once("open", async () => {
  ...
  try {
    ...
    // Extract _id from gameLibrary
    
    // from the gameLibrary, get all the Id's
    // This user (user0) owns a copy of all of the games defined in the gameLibrary.
    data.users[0].userGames = gameLibrary.map(({_id})=> _id);

    // User1 has borrowed a copy of the first game defined in the gameLibrary.
